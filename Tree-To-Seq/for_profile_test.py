...
class ForDataset(Dataset):
    def __init__(self, path, num_vars = 10, num_ints = 11, binarize = False):
        ...
        self.for_data_pairs = [(for_prog, Variable(torch.LongTensor(tree_to_list(lambda_prog) + [alphabet_size + 1]))) for for_prog, lambda_prog in zip(for_progs, lambda_progs)][:10]
