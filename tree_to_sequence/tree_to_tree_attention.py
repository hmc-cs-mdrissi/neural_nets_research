...
class TreeToTreeAttention(nn.Module):
    ...
    def forward_train(self, input_tree, target_tree, teacher_forcing=True, input_tree_form=False):
        """
    ...
    def print_img_tree_example(self, img, tree):
        print("IMAGE")
        print(img.shape)
        print(np.repeat(img[0][0], 3).shape)
        self.display_normally(img[0][0])
        print("WE WANTED")
        pretty_print_tree(tree)
        print("WE GOT")
        pretty_print_tree(self.forward_prediction(img))
